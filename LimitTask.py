...
#fin de While TaskTime<NbMin*60 => le temps limite et dépassé  
  ...
  def FindPID(self,exename):
      p=subprocess.Popen('tasklist /FI "IMAGENAME eq '+exename+'"',stdout=subprocess.PIPE,stderr=subprocess.PIPE,creationflags=subprocess.SW_HIDE,shell=True) #permet de ne pas montrer la console
      a=p.stdout.readlines()
      info=[]
      i=0
      thispid=str(os.getpid())
      while len(a)>3+i and a[3+i].split()!=[]:
...
u"""Attention ! il y 3 arguments à cette commande:
    ...
    Exemple (pour limiter la l'utilisation de la calculatrice à 10 mn toutes les 60mn):
               ...
               >>{} calc.exe 10 60""".format(sys.argv[0].split(os.path.sep)[-1:][0][:-4]), u"Erreur de lancement !",4096) #récupération du dernier élément du fullpath [-1:] d'une liste [0], sans les 4 derniers caractères
    ...
    MyPC=LimitTask()
    while True:  
      process=MyPC.FindPID(os.path.split(sys.argv[0])[1].split('.')[0]+'.exe')
      if process==[]: break
      subprocess.check_output('taskkill /F /PID '+str(process[0][1]))   
